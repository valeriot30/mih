...
int main (int argc, char**argv) {
    ...
    double ct = (double)(end0-start0) / (double) (CLOCKS_PER_SEC);
    double wt = (double)(end1-start1);

    ftime = omp_get_wtime();
    exec_time = ftime - itime;
    printf("\n\nTime taken by parallelizing code is %f \n", exec_time);
    
    printf("done. | cpu %.0fm%.5fs | wall %.0fm%fs\n", ct/60, ct-60*int(ct/60), wt/60, wt-60*int(wt/60));
    ...
    printf("\n\nTime taken by parallelizing code for queries is %f s\n", (end_omp - start_omp) / NQ);
    printf("done | cpu %.5fs | wall %.5fs | VM %.5fgb | RSS %.1fgb     \n", result.cput, result.wt, result.vm, result.rss);
